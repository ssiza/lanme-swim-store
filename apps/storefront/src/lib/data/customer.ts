...
export const retrieveCustomer =
  async (): Promise<HttpTypes.StoreCustomer | null> => {
    logFetchStart("retrieveCustomer")

    const authHeaders = await getAuthHeaders()

    if (!authHeaders) {
      logFetchEnd("retrieveCustomer", { reason: "no_auth", found: false })
      return null
    }
    ...
    return await sdk.client
      ...
      .then(({ customer }) => {
        logFetchEnd("retrieveCustomer", { found: Boolean(customer) })
        return customer
      })
      .catch((error) => {
        logFetchError("retrieveCustomer", error)
        return null
      })
  }
...
export async function resetCustomerPassword(
  ...
  if (!token) {
    return {
      state: "error",
      error: "This reset link is invalid or missing a token.",
    }
  }

  if (!password) {
    return { state: "error", error: "Please enter a new password." }
  }
